   ...
   /**
    ...
    toArrPreorder(node = this.root, vals = []) { 
      if(node) {
        ...
        //check for count and add it that many times if more than one exists
        if(node.count > 1) {
          for (let x = 0; x < node.count; x++) {
            vals.push(node.data);
          }
        }
        else {
        ...
        // vals.push(node.data);     (uncomment if removing check for count)
    ...
    toArrPostorder(node = this.root, vals = []) {
      if(node){
        ...
        // check for count and add it that many times if count is more than 1
        if(node.count > 1) {
          for (let x = 0; x < node.count; x++) {
            vals.push(node.data);
          }
